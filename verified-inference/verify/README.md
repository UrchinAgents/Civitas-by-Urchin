...
```shell
cd ../verify
nitro-cli build-enclave --docker-uri "ghcr.io/urchin-ai/aws_enclave:v0.0.2" --output-file "urchin.eif"
```
...
Congratulations! You have now successfully verified the attestation. This means that the code inside the TEE is exactly the same as in ../enclave sub folder and will be executed exactly as intended without any way for Urchin to interfere.
