...
BASE_URL = "https://api.urchin.com"


class UrchinChatHistory(BaseModel):
    request: Dict
...
def get_history(
    urchin_api_key: str, limit: int = 10, cursor: str = None, filter: str = "all"
) -> List[UrchinChatHistory]:
    if filter not in ["all", "mine"]:
        raise UrchinError("Incorrect filter given, valid values are: 'all', 'mine'")

    query_params = {"limit": limit, "filter": filter}
    if cursor:
        query_params["cursor"] = cursor

    return _make_request(urchin_api_key, query_params)


def get_by_hash(urchin_api_key: str, hash: str) -> Optional[UrchinChatHistory]:
    try:
        url = urljoin(BASE_URL, f"/v1/verified/chat/completions/{hash}")
        headers = {"accept": "application/json", "Authorization": urchin_api_key}
...
def _make_request(
    urchin_api_key: str, query_params: Optional[Dict]
) -> List[UrchinChatHistory]:
    try:
